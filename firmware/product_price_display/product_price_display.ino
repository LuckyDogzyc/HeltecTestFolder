...
<div class="card"><h2>Wi-Fi 配网</h2><div class="muted">保存后设备会立即尝试连接，并返回成功/失败反馈。AP 热点默认：PokemonDisplay-XXXX / 12345678。</div><div class="row"><input id="ssid" placeholder="Wi-Fi SSID"><input id="pass" placeholder="Wi-Fi 密码" type="password"><button onclick="saveWifi()">保存并连接</button></div><div class="row"><button class="secondary" onclick="reconnectWifi()">重新连接 Wi-Fi</button><button class="danger" onclick="clearWifi()">清除 Wi-Fi 设置</button></div><div id="wifiResult" class="muted"></div></div>
<div class="card"><h2>诊断</h2><pre id="diag"></pre></div>
</div><script>
let statusData=null,indexData=null,cards=[];
const $=id=>document.getElementById(id);
function setMsg(t,cls='muted'){const e=$('msg');e.className=cls;e.textContent=t;}
async function api(path,opt){const r=await fetch(path,opt);const j=await r.json();if(!r.ok)throw new Error(j.error||r.status);return j;}
...
function renderStatus(){const s=statusData,c=s.card,w=s.wifi,p=s.power,f=s.feed; $('pid').value=c.productId; $('tpl').value=s.config.template; $('showBat').checked=s.config.showBattery; $('current').innerHTML=`<div class="price">${c.found?'$'+c.marketPrice:'NO DATA'}</div><div><b>${c.name||'未找到卡牌'}</b></div><div class="muted">ID ${c.productId} · Bucket ${f.bucket} · HTTP ${f.httpStatus} · ${f.stage}</div><div><span class="pill">Wi-Fi ${w.connected?'已连接 '+w.ip:'未连接'}</span><span class="pill">RSSI ${w.rssi}</span><span class="pill">${p.source==='battery'?(p.voltage.toFixed(2)+'V'):'USB/未接电池'}</span></div><div class="muted">${c.setName||''} ${c.rarity||''} ${c.subTypeName||''}</div><div class="bad">${f.lastError||''}</div>`; $('diag').textContent=JSON.stringify(s,null,2);}
async function refreshScreen(){setMsg('刷新中，请等待墨水屏完成刷新...');try{const j=await api('/api/refresh',{method:'POST'});statusData=j.status;renderStatus();setMsg(j.ok?'刷新完成':'刷新失败：'+j.error,j.ok?'ok':'bad');}catch(e){setMsg('刷新失败：'+e.message,'bad');}}
...
async function saveWifi(){const body=`ssid=${encodeURIComponent($('ssid').value)}&pass=${encodeURIComponent($('pass').value)}`;$('wifiResult').textContent='连接中...';try{const j=await api('/api/wifi',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});$('wifiResult').className=j.connected?'ok':'bad';$('wifiResult').textContent=j.connected?`连接成功：${j.ip} RSSI ${j.rssi}`:`连接失败：${j.error}`;await loadStatus();}catch(e){$('wifiResult').className='bad';$('wifiResult').textContent='连接失败：'+e.message;}}
async function reconnectWifi(){try{const j=await api('/api/wifi/reconnect',{method:'POST'});$('wifiResult').className=j.connected?'ok':'bad';$('wifiResult').textContent=j.connected?`重新连接成功：${j.ip}`:`重新连接失败：${j.error}`;await loadStatus();}catch(e){$('wifiResult').textContent=e.message;}}
...
async function loadIndex(){if(indexData)return; $('searchInfo').textContent='加载搜索索引中...';try{const url=(statusData&&statusData.feed.searchIndexUrl)||'https://raw.githubusercontent.com/LuckyDogzyc/HeltecTestFolder/main/cards/search_index.min.json';indexData=await (await fetch(url)).json();cards=indexData.cards||[];$('searchInfo').textContent=`索引已加载：${cards.length} 张卡`;searchCards();}catch(e){$('searchInfo').textContent='索引加载失败：'+e.message;}}
...
async function chooseCard(id,rf){$('pid').value=id;await saveProduct(rf);}
loadStatus();
</script></body></html>
)HTML";
...
static void setupRoutes() {
  ...
  server.on("/api/wifi", HTTP_POST, []() {
    ...
    if (!ok) startConfigAP();
    String body = String("{\"connected\":") + (ok ? "true" : "false") + ",\"ip\":\"" + (ok ? WiFi.localIP().toString() : String("")) + "\",\"rssi\":" + (ok ? String(WiFi.RSSI()) : String(0)) + ",\"error\":\"" + jsonEscape(lastError) + "\"}";
    sendJson(ok ? 200 : 500, body);
  });
