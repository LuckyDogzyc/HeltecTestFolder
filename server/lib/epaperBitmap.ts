...
// 动态槽位：位图模式中由固件本地实时绘制的字段（价格/时间/日期/电池等）。
// 电源状态绝不能烘进静态 frame，否则旧的 USB 像素会遮住唤醒后读取的电量。
export const DYNAMIC_FIELDS = ['market', 'low', 'mid', 'high', 'time', 'date', 'power'];
...
export function renderStaticFrame(program: RenderCommand[], card: CardSample, fontFamily = FONT_FAMILY): RenderedFrame {
  ...
  for (const item of program) {
    ...
    // 跳过所有动态槽位（价格/时间/日期——固件每次唤醒用内置字体重画，保证实时）。
    // 固件已内置 9/12/18/24pt 四档字体，任何字号的动态元素都能本地绘制。
    if (isDynamic(item)) continue;
...
// 动态槽位列表（固件本地绘制的元素）：价格/时间/日期，不限字号（固件内置 9/12/18/24pt）
export function dynamicSlots(program: RenderCommand[]) {
  return program
    .filter((item) => item.visible && isDynamic(item))
    .map((item) => ({
      value: item.value,
      x: item.x,
      y: item.y,
